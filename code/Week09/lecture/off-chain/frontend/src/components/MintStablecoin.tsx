...
export default function MintStablecoin() {
  ...
  const getColateralUTxOToUnlock = async () => {
    if (
      ...
      return;
    const colToUnlockUTxO = await findUTxO(lucid, collateralToUnlockUTxORef);
    ...
    if (
      ...
      const tx = await lucid!
        ...
        .payToContract(
          collatealAddr,
          {
            inline: Data.to<CollateralDatum>(collDatum, CollateralDatum),
          },
  ...
  const burnOrLiqSC = async () => {
    ...
    if (
      ...
      const colRed: CollateralRedeemer = burnOrLiq ? "Redeem" : "Liquidate";

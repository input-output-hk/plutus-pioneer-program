...
mkValidator dat () ctx -- FIX ME!
    | checkSig1 && checkDeadline1 = True
    | checkSig2 && checkDeadline2 = True
  where
    info :: TxInfo
    ...
    checkDeadline1 :: Bool
    checkDeadline1 = to (deadline dat) `contains` range
    
    checkDeadline2 :: Bool
    checkDeadline2 = from (1+deadline dat) `contains` range -- 1+dead... after the deadline

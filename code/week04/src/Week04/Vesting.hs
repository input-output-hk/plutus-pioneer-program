...
grab = do
    ...
    utxos <- Map.filter (isSuitable pkh now) <$> utxosTxOutTxAt scrAddress
    if Map.null utxos
  ...
  where
    isSuitable :: PubKeyHash -> POSIXTime -> (ChainIndexTxOut, ChainIndexTx) -> Bool
    isSuitable pkh now (utxo, tx) = case txOutDatumHash $ toTxOut utxo of
        Nothing -> False

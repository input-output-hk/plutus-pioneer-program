...
import           Basement.Compat.Base       hiding (($), (<>))
...
test' c1 c2 = runEmulatorTraceIO' def emCfg $ myTrace c1 c2
  where
    ...
    emCfg = EmulatorConfig (Left (Map.fromList
        [ (knownWallet 1, v <> assetClassValue (AssetClass (gameTokenCurrency, gameTokenName)) 1)
        , (knownWallet 2, v)
        ])) (SlotConfig 1 (POSIXTime 0)) (FeeConfig (Lovelace 100000) (0.002 :: Double))

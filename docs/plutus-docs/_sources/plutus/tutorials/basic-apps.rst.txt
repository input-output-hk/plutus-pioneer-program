...
You can build a schema using the :hsobj:`Plutus.Contract.Endpoint` type family to construct individual endpoint types, and the ``.\/`` operator to combine them.
...
After calling :hsobj:`Plutus.Contract.submitTxConstraints` in the next line, the Plutus app runtime examines the transaction constraints ``tx`` and builds a transaction that fulfills them.
...
Every contract in the Plutus apps emulator must define its public interface like this:
...
The :hsobj:`Plutus.Contract.selectList` function acts like a choice between two branches.
...
The ``forever`` call, which runs the application in an infinite loop, is necessary for the
:hsobj:`Plutus.Trace.Emulator.EmulatorTrace`.
...
You can now compile the contract and create a simulation.
...
.. literalinclude:: BasicAppsSpec.hs
   :start-after: BLOCK1
   :end-before: BLOCK2

You should see an output similar to what follows:

.. literalinclude:: basic-apps-trace.txt

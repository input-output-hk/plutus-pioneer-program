...
The ``GameModel`` type must be an instance of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel`
...
A generated test is called :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Actions`, and is, as the name suggests, essentially a
sequence of :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` values. We can
run tests by using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.propRunActions_`:
...
transferred correctly, and contracts didn't crash.
  ...
  .. note::
     ...
     There is also a more general function :hsobj:`Plutus.Contract.Test.ContractModel.Interface.propRunActions` that allows
     the check at the end of each test to be customized.

But how does ``quickCheck`` know what code to run when you check ``prop_Game``?
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.propRunActions_` needs to create a handle
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey`, another
associated datatype of the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class; we talk to a
contract instance by referring to its
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey`. The
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey` type is
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey` is defined as a
GADT.

In this particular case, there is only one type of contract under
test, and so it suffices to define a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey` type with a single
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.initialInstances`,
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.instanceWallet`, and
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.instanceContract` fields of the
``ContractModel`` class:
...
specify how to generate an :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action`, which we will do next.
...
Now we can define a generator for :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action`, as a method of the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class:
...
our generation; if any :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` disappears altogether, or is generated
...
correct. An important part of a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` defines how funds
...
type parameter of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class. (Note that this contract
...
test case, :hsobj:`Plutus.Contract.Test.ContractModel.Interface.initialState`, and a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState` function to model the way
we expect each operation to change the state. These are both methods
in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class.
...
The :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState` function is defined in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Spec` monad
...
contract state (using ``%=`` and generated ``Lens`` operations),
mints the game :term:`token` (using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.mint`), deposits it in the creator's
wallet, and withdraws the Ada locked in the contract (using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.deposit`
and :hsobj:`Plutus.Contract.Test.ContractModel.Interface.withdraw`):

.. literalinclude:: GameModel.hs
   :start-after: START nextState Lock v1
   :end-before: END nextState Lock v1

A :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` actually tracks not only the contract model state (in
...
test. It is these expectations that are manipulated by :hsobj:`Plutus.Contract.Test.ContractModel.Interface.mint`,
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.deposit`, etc... don't confuse them with operations that *actually*
mint or move :term:`tokens<token>` in the implementation. The :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ModelState` type
contains all of this information.


When making a guess, we need to check parts of the contract state
(which we read using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.viewContractState`), and then we update the
stored password, game value, and wallet contents appropriately.

.. literalinclude:: GameModel.hs
   :start-after: START nextState Guess v1
   :end-before: END nextState Guess v1

``GiveToken`` just transfers the game :term:`token` from one wallet to another using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.transfer`.

.. literalinclude:: GameModel.hs
   :start-after: START nextState GiveToken v1
   :end-before: END nextState GiveToken v1

At the end of each test, the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` framework checks that
every wallet contains the :term:`tokens<token>` that the model says it should.

We can exercise the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState` function already by generating and
...
not yet been minted--so the ``fromJust`` in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState` function
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.precondition` method to our
...
The :hsobj:`Plutus.Contract.Test.ContractModel.Interface.precondition` is parameterised on the entire model state, which
...
Then we define the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform` method of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class:

.. literalinclude:: GameModel.hs
   :start-after: START perform type
   :end-before: END perform type

The job of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform` method in this case is just to invoke the
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform` lets us find the contract handle associated with each
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractInstanceKey`.

For the most part, it is good practice to keep the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform` function
...
to :term:`contract endpoints<contract endpoint>` makes interpreting test failures much easier.
 ...
 .. note::
    ...
    **Helping shrinking work better by choosing test case actions well**
    ...
    In the definition of :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform` above, the ``GiveToken`` action is a little
    ...
    *from*. So why did we choose to define a ``GiveToken w`` action to
    ...
    would correspond more directly to the code in :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform`?
...
specify this shrinking by defining the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.shrinkAction` operation in the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class:
...
as replacements for the given :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action`, when QuickCheck is simplifying
...
We add a call to ``delay`` in each branch of :hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform`:
...
emulator. We do this using corresponding calls to :hsobj:`Plutus.Contract.Test.ContractModel.Interface.wait` in the
definition of :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState`:
...
generated. By default, testing a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` just displays the
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitoring` method in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class:

.. literalinclude:: GameModel.hs
   :start-after: START monitoring type
   :end-before: END monitoring type

This function is called for every :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` in a test case,
and given the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ModelState` before and after the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action`. Its result is a function that is applied to the
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitoring` as
...
possibilities (and the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.precondition` for guesses does not depend on
...
generators are little programs in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` monad, such as
this one:
...
This :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` fragment simply specifies a unit test in terms
of the underlying :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` we have already seen,
using :hsobj:`Plutus.Contract.Test.ContractModel.Interface.action` to include a specific
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` in the test. To run such a test, we must specify
...
which uses :hsobj:`Plutus.Contract.Test.ContractModel.Interface.forAllDL` to generate a test sequence from the
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel`.
...
As well as writing unit tests in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` monad, we can add random
...
Here :hsobj:`Plutus.Contract.Test.ContractModel.Interface.forAllQ` lets us generate a random value using
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.forAllQ` takes a
...
information. Every such test contains a list of :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action`, tagged
``Do``, and *witnesses*, tagged ``Witness``. The witnesses record the
results of random choices made by :hsobj:`Plutus.Contract.Test.ContractModel.Interface.forAllQ`: in this case, the Ada
...
the ``gameValue`` is only 1 Ada, so of course we cannot withdraw 3.
 ...
 .. note::
    ...
    :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel`, then QuickCheck only generates actions whose
    :hsobj:`Plutus.Contract.Test.ContractModel.Interface.precondition` is satisfied. On the other hand, when we use dynamic
...
modified :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` test, so it is not a counterexample to the property we
are testing.
...
for ever. We could try to specify this with a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` test that requires that *no
Ada remain locked in the contract after any sequence of actions*. We
can include a random sequence of actions in a :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` test using
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.anyActions_`, and we can make assertions about the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ModelState` using
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.assertModel`. Thus we can define
...
locked (:hsobj:`Plutus.Contract.Test.ContractModel.Interface.lockedValue` extracts the total value locked in contracts from
the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.ModelState`).
...
``BadPrecondition`` (for the assertion).
...
contract. This is the effect of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState` function in our model,
...
strategy. To find out, we can :hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitor` the contract model during our
tests. As in the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitoring` method of the
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.ContractModel` class, we
can use any of the QuickCheck operations for analyzing test cases, but
instead of applying the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitoring` at every action in a test case, we
can :hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitor` at selected points.
...
With the addition of the :hsobj:`Plutus.Contract.Test.ContractModel.Interface.monitor` line, QuickCheck tells us what
...
hitherto. The :hsobj:`Plutus.Contract.Test.ContractModel.Interface.DL` monad is an instance of ``Alternative``, so we can
write tests with random control flow, weight choices suitably, and so
on. For example, :hsobj:`Plutus.Contract.Test.ContractModel.Interface.anyActions`, which generates a random sequence of
...
alternative is guarded by :hsobj:`Plutus.Contract.Test.ContractModel.Interface.stopping`, which means it will be chosen
*only* if the test case is 'getting too long'; in this case
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.anyActions` will generate an empty sequence. We can exercise fine
...
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` to complete before performing the next
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` (because we inserted calls to ``delay`` into
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.perform`, and
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.wait` into
:hsobj:`Plutus.Contract.Test.ContractModel.Interface.nextState`). In reality, different wallets may perform
...
explicit :hsobj:`Plutus.Contract.Test.ContractModel.Interface.Action` in test cases instead. However,
